...
class Runner {
    ...
    Runner(GameEngine model, Pane pane, double width, double height) {
        ...
        this.score = new Text(10, 20, "Score for this level: " + model.getCurrentLevel().getTargetTime());
        ...
        this.totalScore.setX(30);
        this.totalScore.setY(60);
        this.pane.getChildren().add(this.totalScore);
    ...
    private void draw() {
        model.tick();
        double s = this.model.getLevelScore();
        if(s < 0) {
            s = 0;
        }

        if (model.isFinished()) {
            pane.getChildren().remove(this.score);
            pane.getChildren().remove(this.totalScore);
            pane.getChildren().remove(this.lives);
            drawScreen("You Won in " + model.getDuration().toSeconds() + "s and\nhad "
                    + model.getLives() + " lives remaining!\nYour final score was " + Math.round(model.getTotalScore()));
            return;
        ...
        this.score.setText("Score for this level: " + s);
